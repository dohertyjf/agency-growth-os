...
export default async function LeadGoalListPage() {
  const session = await auth()
  if (!session) redirect("/auth/signin")
  if (session.user.role !== "coach") redirect("/dashboard")

  const leads = await prisma.leadGoalSubmission.findMany({ orderBy: { createdAt: "desc" } })
    ...
    <div>
      ...
      {leads.length === 0 ? (
        <div style={{ background: "#fff", border: "1px solid #ECE7DE", borderRadius: 12, padding: 40, textAlign: "center", color: "#9C9590", fontSize: 14 }}>
          No submissions yet. Entries from the public calculator will appear here.
        </div>
      ) : (
        <div style={{ background: "#fff", border: "1px solid #ECE7DE", borderRadius: 12, overflow: "hidden" }}>
          {leads.map((lead, i) => {
            let result: LeadsGoalResult | null = null
            try { result = JSON.parse(lead.result) as LeadsGoalResult } catch { /* ignore */ }
            const needed = result ? Math.ceil(result.leadsToReachGoal) : null
            return (
              <div key={lead.id} style={{ display: "flex", alignItems: "center", borderTop: i === 0 ? "none" : "1px solid #F1ECE3" }}>
                <Link href={`/leads/lead-goal/${lead.id}`}
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#E9532A", fontVariantNumeric: "tabular-nums" }}>{needed != null ? `${needed}/mo` : "—"}</div>
                    <div style={{ fontSize: 11, color: "#9C9590" }}>leads needed</div>
                  </div>
                ...
                <div style={{ padding: "0 14px", flexShrink: 0 }}>
                  <DeleteLeadButton endpoint={`/api/lead-goal/${lead.id}`} />
                </div>
              </div>
            )
          })}
