...
export default function ImportClient() {
  ...
  const canImport = !!parsed && !!parsed.client.name && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(parsed.client.email)

  async function runImport() {
    if (!parsed) return
    setImporting(true); setError(""); setProgress("Creating client…")
    try {
      ...
      setError((e as Error).message + ". The client was created; you can re-run the failed section from its tab.")
      setImporting(false)
    }
  }
  ...
  return (
    <div style={{ maxWidth: 780 }}>
      ...
      <p style={{ fontSize: 14, color: "#6F6B64", margin: "0 0 20px", lineHeight: 1.5 }}>
        Upload a completed <strong>Client Intake Template</strong> (.xlsx). It creates the client and imports their accounts &amp; projects, monthly metrics, and team in one step.
      </p>
      ...
      {parsed && (
        <>
          <div style={cardStyle}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#9C9590", marginBottom: 10 }}>Client</div>
          ...
          {!canImport && !importing && (
            <div style={{ fontSize: 13, color: "#C2410C", marginTop: 10 }}>Add the client&apos;s name and a valid email in the Start Here tab, then re-upload.</div>
          )}
        </>
      )}
    </div>
