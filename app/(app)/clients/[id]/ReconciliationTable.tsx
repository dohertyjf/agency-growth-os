...
export default function ReconciliationTable({ contracts, initialAccountMonths, onRevenueUpdate }: Props) {
  const [accountMonths, setAccountMonths] = useState<AccountMonth[]>(initialAccountMonths)
  const [editing, setEditing] = useState<{ contractId: string; month: string; value: string } | null>(null)
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #ECE7DE", borderRadius: 12, padding: 20 }}>
      ...
      <div style={{ fontSize: 11, color: "#9C9590", marginBottom: 14 }}>
        Forecast in muted · click a cell to enter the actual billed amount
      </div>
        ...
        <table style={{ borderCollapse: "collapse", width: "100%", minWidth: "max-content" }}>
          ...
          <tbody>
                  ...
                  return (
                    ...
                    >
                      {isEditing ? (
                        ...
                        <div
                          onClick={() => setEditing({ contractId: contract.id, month, value: am ? String(am.actual) : String(contract.monthly) })}
                          style={{
            ...
            <tr style={{ background: "#F8F6F2" }}>
              <td style={{ ...labelStyle, fontWeight: 700, color: "#1A1916" }}>Total</td>
              {months.map(month => {
                let total = 0
