...
export default async function ClientTabPage({ params }: { params: Promise<{ slug: string; tab: string }> }) {
  ...
  const accountPulses = await prisma.accountPulse.findMany({ where: { account: { clientId: id } } })
  ...
  return (
    <ClientPageClient
      ...
      initialPulses={accountPulses.map(p => ({ accountId: p.accountId, month: p.month, score: p.score, note: p.note ?? null }))}
      goal={goal}
