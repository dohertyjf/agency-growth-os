        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 640 }}>
          {programs.map(p => (
            <div key={p.id} style={{ background: "#fff", border: "1px solid #ECE7DE", borderRadius: 10, padding: "14px 18px" }}>
              ...
              {expandedId === p.id && (
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid #F5F1EC", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 6 }}>
                  {allClients.map(c => (
                    <label key={c.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1A1916", cursor: "pointer" }}>
                      <input type="checkbox" checked={p.clientIds.includes(c.id)} onChange={() => toggleClient(p.id, c.id, p.clientIds)} style={{ accentColor: "#E9532A" }} />
                      {c.name}
                    </label>
                  ))}
                </div>
